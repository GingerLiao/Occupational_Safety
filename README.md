# 職盾 JobShield — 企業職業安全風險管理平台

> 一個專為**企業**打造的職安風險管理工具。整合政府審查名單、違規處分、全國法規、司法院判決與職災專責醫院名單，運用 Hybrid RAG 技術，協助企業於**發包前評估供應商風險**、**職災後掌握法律責任與應變流程**，建立可持續的合規管理（SaaS）。

本專案為國立中央大學初賽提案「職盾」的可運作網站實作（亮色系 / 企業端聚焦版）。
...
| 🏭 **供應商風險評估** | 即時比對「通過職安審查名單」與「違規處分紀錄」，輸出職安評分、風險評級、發包建議，並生成**風險分析拓樸圖**（Mermaid）。 |
| 📊 **多供應商並排比較** | 將候選供應商加入追蹤，發包前同時評比數家風險等級，自動標示最佳選擇。 |
| 🏥 **員工就醫指引** | 職災後引導員工前往最近的職業傷病專責醫院（Leaflet 地圖），提供**導航、聯絡分機、一鍵撥號**，保全醫療證據。 |
| 🧮 **賠償風險試算** | 依《勞工職業災害保險及保護法》即時試算職災傷病給付與企業潛在補償責任（前 60 日 100%、第 61 日起 70%、上限 730 日）。 |
| 📄 **文件自動生成** | 由對話內容自動帶入，一鍵生成《職業災害事故通報表》PDF（html2canvas + jsPDF，支援中文）。 |
| 🎤 **語音輸入** | Web Speech API，依介面語言自動切換辨識語系。 |
| 🌐 **雙語介面** | 中文 / English 全站切換（國際供應鏈、ESG 導向）。 |

> UI 採**亮色系**專業設計；介面語言切換中／英時，所有介面字串同步更新（政府資料如公司名、法條本身屬原文資料維持中文）。
