...
const App = {
  ...
  go(view){
    ...
    document.getElementById("topTitle").textContent = titles[view]||"職盾 JobShield";
    window.scrollTo(0,0);
    // lazy init
...
const FEATURES = [
  ...
  {view:"chat",   key:"lang",   ic:`<circle cx="12" cy="12" r="10"/><path d="M2 12h20M12 2a15 15 0 0 1 0 20M12 2a15 15 0 0 0 0 20"/>`},
];
...
window.addEventListener("DOMContentLoaded", async ()=>{
  ...
  document.getElementById("covCount").textContent = `${DB.hospitals.length} / ${DB.companies.length}`;
  ...
  window.addEventListener("langchange", ()=>{ renderFeatures(); renderMobileNav(); applyI18n();
