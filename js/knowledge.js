...
const LocalRAG = {
  ...
  async answer(qRaw){
    ...
    // 意圖判斷
    const isVendor = /評估|發包|供應商|供應商|外包|承攬|合作|風險/.test(q) && !/我|員工|受傷|墜樓|墜落|理賠|給付/.test(q.slice(0,6));
  ...
  generalAnswer(q, updated){
    ...
    md += `- 🏭 **供應商風險評估** — 發包前比對職安審查與違規名單\n`;
    md += `- ⚖️ **理賠與判決查詢** — 職災給付與賠償金額預估\n`;
    md += `- 🏥 **醫療指引** — 最近的職業傷病專責醫院\n`;
    md += `- 📄 **文件生成** — 職災事故通報表\n\n`;
    md += `請告訴我更多細節，例如供應商名稱、事故經過、或您的投保薪資。`;
    md += this.srcBlock([{tag:"資料庫",text:"政府公開資料平台 / 全國法規資料庫 / 勞動部 / 司法院判決書系統"}], 70, updated);
    md += this.fuBlock(["評估一家發包供應商的風險","我在工地受傷了，該怎麼辦？","找最近的職災專責醫院"]);
    return md;
  },
};
